...
extern "C" __declspec(dllexport) AcRx::AppRetCode acrxEntryPoint(AcRx::AppMsgCode msg, void* pPkt)//16-4-10 22:34
//*****YLH****导入
{
...
ArxDbgApp::entryPoint(AcRx::AppMsgCode msg, void* pkt)
{
    ...
    else if (msg == AcRx::kNoDependencyMsg) {// kNoDependencyMsg = 11,
    ...
    else if (msg == AcRx::kOleUnloadAppMsg) {//kOleUnloadAppMsg = 12,
        acutPrintf(_T("\nAPPMSG: %s, kOleUnloadAppMsg"), m_appName);
        return AcRx::kRetOK;
    }
    else if (msg == AcRx::kPreQuitMsg) {// kPreQuitMsg      = 13
        acutPrintf(_T("\nAPPMSG: %s, kPreQuitMsg"), m_appName);
        return AcRx::kRetOK;
    }
    else if (msg == AcRx::kInitDialogMsg) {// kInitDialogMsg   = 14,
        acutPrintf(_T("\nAPPMSG: %s, kInitDialogMsg"), m_appName);
        return initDialog(pkt);
    }
    else if (msg == AcRx::kEndDialogMsg) {//kEndDialogMsg    = 15,
...
ArxDbgApp::startSplashScreen()
{
	...
	if (success) {
		...
		acDocManager->pushResourceHandle(dllInstance());/*－－在构造函数中加载菜单资源
														acDocManager->pushResourceHandle(_hdllInstance); // 切换当前使用的资源，千万不要忘记加上！_hdllInstance是模块*/
